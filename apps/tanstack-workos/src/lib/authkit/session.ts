...
export async function withAuth() {
	...
	const { sid: sessionId, org_id: organizationId, role, permissions, entitlements } = decodeJwt<AccessToken>(session.accessToken);
...
export async function updateSession(request: Request, options: AuthkitOptions = { debug: false }): Promise<AuthkitResponse> {
	const session = await getSessionFromCookie();
	...
	try {
		...
		const { sid: sessionId, org_id: organizationId, role, permissions, entitlements } = decodeJwt<AccessToken>(accessToken);
